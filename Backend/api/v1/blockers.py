...
async def report_mid_day_blocker(payload: BlockerCreate, db: AsyncSession = Depends(get_db)):
...
async def clear_active_blocker(blocker_id: int, db: AsyncSession = Depends(get_db)):
    repo = BlockerRepository(db)
